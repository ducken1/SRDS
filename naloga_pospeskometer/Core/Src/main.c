...
/* Configure the peripherals common clocks */
  ...
  initOrientation();
  i2c1_pisiRegister(0x19, 0x20, 0x47); //nastavi odzivnost na 50Hz (01) ter 111 na koncu za aktivacijo xyz 47 ker 0100 = 4 | 0 1 1 1 = 7.. v registru CTR_REG1_A
  i2c1_pisiRegister(0x19, 0x23, 0x10); //nastavi obcutljivost na +/- 4g; ... 10 ker 0001 = 1 | 0000 = 0 v registru CTRL_REG4_A
  ...
  while (1)
  {
	  ...
	  if (prenosAscii) {
		  i2c1_beriRegistre(0x19, 0x28,(uint8_t*)&meritev[2], 2); // OUT X 0x28
		  i2c1_beriRegistre(0x19, 0x2A,(uint8_t*)&meritev[3], 2); // OUT Y 0x2A
		  i2c1_beriRegistre(0x19, 0x2C,(uint8_t*)&meritev[4], 2); //OUT Z 0x2C
		  HAL_Delay(100);
		  meritev[1] = numPackage;
		  numPackage = numPackage +1;

		  uint8_t size = sprintf(AsciiPodatki, "{\"ACC\":%d, \"X\":%.3f,\"Y\":%.3f,\"Z\":%.3f}\n\r", meritev[1], (float)(meritev[2])*0.000125, (float)(meritev[3])*0.000125, (float)(meritev[4])*0.000125); // delimo z 8000 za pretvorbo v gravitacijski pospesek
